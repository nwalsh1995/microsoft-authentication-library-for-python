...
from .oauth2cli import JwtAssertionCreator
from .oauth2cli.async_oauth2 import AsyncClient
...
class AsyncClientApplication(object):
    ...
    def __init__(
        ...
        self.authority = authority_obj if authority_obj is not None else AsyncAuthority(
    ...
    @classmethod
    async def initialize(cls, client_id,
                         client_credential=None, authority=None, validate_authority=True,
                         token_cache=None,
                         verify=True, proxies=None, timeout=None,
                         client_claims=None, app_name=None, app_version=None, authority_obj=None, oauth_client=None):
        authority = authority_obj if authority_obj is not None else await AsyncAuthority.initialize(
            authority or "https://login.microsoftonline.com/common/",
            validate_authority, verify=verify, proxies=proxies, timeout=timeout)

        return cls(
            client_id, client_credential=client_credential, validate_authority=validate_authority,
            token_cache=token_cache, verify=verify, proxies=proxies, timeout=timeout, client_claims=client_claims, app_name=app_name,
            app_version=app_version, authority_obj=authority, oauth_client=oauth_client
        )
    ...
    def _build_client(self, client_credential, authority):
        ...
        if isinstance(client_credential, dict):
            ...
            client_assertion_type = AsyncClient.CLIENT_ASSERTION_TYPE_JWT
        else:
    ...
    async def get_authorization_request_url(
            self,
        ...
        if authority_obj is not None:
            ...
            the_authority = await AsyncAuthority.initialize(authority, verify=self.verify, proxies=self.proxies, timeout=self.timeout)\
                if authority else self.authority

        client = oauth_client if oauth_client is not None else AsyncClient(
    ...
    async def acquire_token_by_authorization_code(
        ...
        return await self.client.obtain_token_by_authorization_code(
    ...
    async def get_accounts(self, username=None):
        ...
        if not accounts:  # Now try other aliases of this authority instance
            for alias in await self._get_authority_aliases(self.authority.instance):
    ...
    async def _get_authority_aliases(self, instance):
        if not self.authority_groups:
            with httpx.AsyncClient(headers={'Accept': 'application/json'},
                                   verify=self.verify, proxies=self.proxies, timeout=self.timeout) as client:
                resp = await client.get("https://login.microsoftonline.com/common/discovery/instance?api-version=1.1&authorization_endpoint=https://login.microsoftonline.com/common/oauth2/authorize")
    ...
    async def acquire_token_silent(
        ...
        """
        result = await self.acquire_token_silent_with_error(
            scopes, account, authority, force_refresh, **kwargs)
        return result if result and "error" not in result else None

    async def acquire_token_silent_with_error(
            self,
            scopes,  # type: List[str]
            account,  # type: Optional[Account]
            authority=None,  # See get_authorization_request_url()
        ...
        #     verify=self.verify, proxies=self.proxies, timeout=self.timeout,
        #     ) if authority else self.authority
        result = await self._acquire_token_silent_from_cache_and_possibly_refresh_it(
        ...
        final_result = result
        for alias in await self._get_authority_aliases(self.authority.instance):
            the_authority = await AsyncAuthority.initialize(
    ...
    async def _acquire_token_silent_from_cache_and_possibly_refresh_it(
            self,
            scopes,  # type: List[str]
            account,  # type: Optional[Account]
        ...
        return await self._acquire_token_silent_by_finding_rt_belongs_to_me_or_my_family(
            authority, decorate_scope(scopes, self.client_id), account,
            force_refresh=force_refresh, **kwargs)

    async def _acquire_token_silent_by_finding_rt_belongs_to_me_or_my_family(
        ...
        if not app_metadata:  # Meaning this app is now used for the first time.
            ...
            at = await self._acquire_token_silent_by_finding_specific_refresh_token(
                authority, scopes,
        ...
        return await self._acquire_token_silent_by_finding_specific_refresh_token(
    ...
    async def _acquire_token_silent_by_finding_specific_refresh_token(
        ...
        for entry in matches:
            ...
            response = await client.obtain_token_by_refresh_token(
...
class PublicClientApplication(AsyncClientApplication):  # browser app or mobile app
    ...
    async def initiate_device_flow(self, scopes=None, **kwargs):
        ...
        """
        correlation_id = _get_new_correlation_id()
        flow = await self.client.initiate_device_flow(
...
class AsyncConfidentialClientApplication(AsyncClientApplication):  # server-side web app

    async def acquire_token_for_client(self, scopes, **kwargs):
        ...
        return await self.client.obtain_token_for_client(
    ...
    async def acquire_token_on_behalf_of(self, user_assertion, scopes, **kwargs):
        ...
        return await self.client.obtain_token_by_assertion(  # bases on assertion RFC 7521
            user_assertion,
